...
from eval_src.pyext2 import RuntimeModule
import signal
import numpy as np
...
def run_test(sample, test=None, debug=False, n_cases=None):
    """
    ...
    inputs_list = []
    outputs_list = []
    assert len(in_outs["inputs"]) == len(in_outs["outputs"]), "inputs and outputs must have the same length"
...
def execute_cb_code(method, inputs_list, outputs_list, timeout, early_stop=False, debug=False):
    ...
    def _compare_outputs(exec_outputs, outputs):
    # ground truth sequences are not tuples
        if isinstance(exec_outputs, tuple):
            exec_outputs = list(exec_outputs)
    
        _tmp_result = exec_outputs == outputs
        if isinstance(outputs, list) and outputs:
            _tmp_result = _tmp_result or (exec_outputs == outputs[0])
    ...
    for index, inputs in enumerate(inputs_list):
        ...
        try:
            ...
            tmp_result = _compare_outputs(exec_outputs, outputs)
            if tmp_result:
        ...
        signal.alarm(0)
        if debug:
            print(f"exec_outputs = {repr(exec_outputs)}, test_outputs = {repr(outputs)}, inputs = {inputs}, {type(inputs)}, "
                f"test_result = {_compare_outputs(exec_outputs, outputs)}")
                #   f"{exec_outputs == [outputs]}")
            debug_infos[index] = {
                    'inputs': inputs,
                    'gt_outputs': outputs,
...
def print_debug_info(inputs, outputs, exec_outputs):
    ...
    comparison_result = compare_std_results(exec_outputs, outputs, debug=False)
    if not isinstance(inputs, list):
        print(f"exec output = {repr(exec_outputs)}\n"
              f"test outputs = {repr(outputs)}\n"
              f"inputs = {inputs.replace(nl,' new-line ')}\n"
              f"type = {type(inputs)}\n"
              f"test result = {comparison_result}")
    else:
        print(f"exec output = {repr(exec_outputs)}\n"
              f"test outputs = {repr(outputs)}\n"
              f"inputs = {inputs}\n"
