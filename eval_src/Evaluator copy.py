...
class Evaluator:
    ...
    def find_pass_code(self, completions: List[str], prior_weights: List[float] = None):
...
class MBPPEvaluator(Evaluator):
    ...
    def check_code_pass(self, code: str, test: list):
        test_code = f"{code}\n" + "\n".join(test)
        
        with Manager() as manager:
            result_dict = manager.dict()
            process = Process(target=self.exec_code, args=(test_code, result_dict))
            process.start()
            ...
            if process.is_alive():
                ...
                return result_dict['result'] == 'passed'
    ...
    def extract_answer_from_model_completion(self, completion: str):
        ...
        preds = completion
        code_maker = "The code is: \[code_begin\](.*?)\[code_end\]"
