...
class MCTS_Searcher:
    ...
    def _backpropagate(self, path: List[MCTS_Node]):
        ...
        parent_node = None
        for node in path[:-1]:
            if node.node_type == Node_Type.USER_QUESTION:
                parent_node = node
                continue
            if node.node_value is None:
                node.node_value = 0
            node.node_value += reward
            for key, val in parent_node.solution_trace[0]["ost_step_value"].items():
                node.solution_trace[0]["ost_step_value"][key] = val
            last_key = list(node.solution_trace[0]["ost_step_value"].keys())[-1]
            node.solution_trace[0]["ost_step_value"][last_key] = node.node_value
            parent_node = node
        node = path[-1]
        for key, val in parent_node.solution_trace[0]["ost_step_value"].items():
            node.solution_trace[0]["ost_step_value"][key] = val
